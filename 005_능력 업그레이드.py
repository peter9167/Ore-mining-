...
while 실행여부:
    ...
    for 이벤트 in pg.event.get():
        if 이벤트.type == pg.QUIT:
            ...
            if 이벤트.key == pg.K_z:
                if 소울곰파워업그레이드횟수 <= 소울곰파워업그레이드최대횟수 and 코인 >= 소울곰파워업그레이드횟수:
                    코인 -= 소울곰파워업그레이드횟수
                    소울곰파워업그레이드횟수 += 1
                    소울곰파워 += 0.4
            elif 이벤트.key == pg.K_x:
                if 소울곰속도업그레이드횟수 <= 소울곰속도업그레이드최대횟수 and 코인 >= 소울곰속도업그레이드횟수:
                    코인 -= 소울곰속도업그레이드횟수
                    소울곰속도업그레이드횟수 += 1
                    소울곰속도 += 40
            elif 이벤트.key == pg.K_c:
                if 광석자동생성업그레이드횟수 <= 광석자동생성업그레이드최대횟수 and 코인 >= 광석자동생성업그레이드횟수:
                    코인 -= 광석자동생성업그레이드횟수
                    광석자동생성업그레이드횟수 += 1
                    광석자동생성시간 -= 1
